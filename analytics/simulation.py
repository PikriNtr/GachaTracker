...
def run_monte_carlo_simulation(pulls: List[Pull], num_sims: int = 10000) -> Dict[str, Any]:
    """Runs 10,000 Monte Carlo gacha simulations and calculates user's statistical luck percentile."""
    ...
    for _ in range(num_sims):
        ...
        while featured < target_featured:
            p_count += 1
            pity += 1
            prob = 0.008 if pity <= 60 else (0.008 + (pity - 60) * 0.05 if pity < 80 else 1.0)
        ...
        if player_pities:
            sim_avg_pities.append(sum(player_pities) / len(player_pities))
        else:
            sim_avg_pities.append(62.5)
...
def generate_simulation_chart(sim_results: Dict[str, Any], player_id: str) -> io.BytesIO:
    ...
    ax.set_title(f'Monte Carlo Pity Distribution (10,000 Players)  •  Player {player_id}', color='#FFFFFF', fontsize=11, fontweight='bold', pad=12)
